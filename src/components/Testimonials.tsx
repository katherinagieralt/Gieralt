...
export function Testimonials() {
  ...
  const highlightMetrics = (text: string) => {
    ...
    return parts.map((part, i) => 
      regex.test(part) ? <span key={i} className="text-rose-500 font-bold">{part}</span> : part
    );
  };
        ...
        if (snapshot.empty) {
          setTestimonials([
            ...
            },
            {
              ...
              name: "Marek Kowalski",
              role: "CEO, TechFlow",
              content: "Współpraca z Katarzyną to czysta przyjemność. Jej podejście do UX zmieniło sposób, w jaki nasi klienci postrzegają nasz produkt. Landing page, który dla nas zaprojektowała, zwiększył naszą konwersję o ponad 40%. Design jest po prostu obłędny.",
              rating: 5,
