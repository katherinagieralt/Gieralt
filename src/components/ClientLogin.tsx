...
export function ClientLogin() {
  ...
  return (
    <div className="min-h-screen bg-transparent flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-blue-500/10 dark:bg-blue-500/20 rounded-full blur-[120px] opacity-50 pointer-events-none" />
      
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
        className="max-w-md w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 text-center shadow-2xl shadow-blue-500/5 relative z-10"
      >
        <div className="mb-6">
          <div className="w-16 h-16 bg-blue-50 dark:bg-blue-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-blue-100 dark:border-blue-500/20 shadow-sm">
            <UserCircle className="w-8 h-8 text-blue-500" />
          </div>
          <h1 className="text-3xl font-display font-semibold text-slate-900 dark:text-white mb-3">Portal <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-blue-400">Klienta</span></h1>
          <p className="text-slate-700 dark:text-slate-300 font-light">Zaloguj się, aby śledzić postępy swojego projektu.</p>
        </div>
